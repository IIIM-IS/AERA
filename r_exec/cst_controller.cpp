...
void CSTController::abduce_simulated(HLPBindingMap *bm, Fact *f_super_goal) {
  ...
  for (auto o = overlays_.begin(); o != overlays_.end(); ++o) {
    if (((CSTOverlay*)(*o))->is_simulated())
      // Skip overlays that produced during simulated forward chaining.
      continue;

    HLPBindingMap bm_copy(bm);
    // Debug: this is inefficient. We want to merge (*o)->binding_ into bm_copy, but use an icst.
...
void CSTController::abduce(HLPBindingMap *bm, Fact *f_super_goal) {
  ...
  Sim *sub_sim;
  if (sim->get_mode() == SIM_ROOT)
    sub_sim = new Sim(opposite ? SIM_MANDATORY : SIM_OPTIONAL, sim->get_thz(), f_super_goal, opposite, sim->root_, 1, sim->solution_controller_, sim->get_solution_cfd(), Timestamp(seconds(0)));
  else
